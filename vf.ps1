...
The launcher for Vocal Forge.

.DESCRIPTION

This script is the entry point for Vocal Forge. It is responsible for installing dependencies,
...
update:    Update Vocal Forge to the latest version.
uninstall: Uninstall dependencies and user generated data.
run:       Start Vocal Forge.
dev:       Start Vocal Forge in development mode.
cli:       Start Vocal Forge in CLI mode.
...
function Main {
    ...
    switch ($Command) {
        "install" {
            ...
            uv run ./src/vocal_forge/core/main.py
        }
        "update" {
            git pull
        }
        ...
        "run" {
            Assert-Dependencies
            uv run ./src/vocal_forge/web/main.py @Arguments
        }
        "dev" {
            Assert-Dependencies
            uv run gradio ./src/vocal_forge/web/main.py --demo-name app
        }
        "cli" {
            Assert-Dependencies
            uv run ./src/vocal_forge/cli/main.py @Arguments
        }
        "docs" {
            Assert-Dependencies
